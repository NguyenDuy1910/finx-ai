...
def merge_weight_hints(
    ...
    """Merge LLM weight hints with default weights for the given intent.

    Returns a dict of weight values. In the paper-aligned design, these
    weights are simplified — the old 5-dimensional reranker is replaced
    by the paper's 3-source retrieval + EBR scoring.
    """
    # Default weights per intent (simplified from old reranker)
    DEFAULTS = {
        "text_match": 0.4,
        "graph_relevance": 0.3,
        "data_quality": 0.1,
    ...
    return {
        "text_match": hints.text_match if hints.text_match is not None else DEFAULTS["text_match"],
        "graph_relevance": hints.graph_relevance if hints.graph_relevance is not None else DEFAULTS["graph_relevance"],
        "data_quality": hints.data_quality if hints.data_quality is not None else DEFAULTS["data_quality"],
        "usage_frequency": hints.usage_frequency if hints.usage_frequency is not None else DEFAULTS["usage_frequency"],
        "business_context": hints.business_context if hints.business_context is not None else DEFAULTS["business_context"],
    }
